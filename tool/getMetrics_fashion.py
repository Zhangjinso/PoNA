...
def load_generated_images(images_folder):
    ...
    for img_name in os.listdir(images_folder):
       ...
       # pa = '/home/zjs/grade2/impersonator/outputs/fashion/imitators'
       # if img_name not in os.listdir(pa):
       #     continue;
        input_images.append(addBounding(img[:, :w]))
        target_images.append(addBounding(img[:, 2*w:3*w]))
        generated_images.append(addBounding(img[:, 4*w:]))
        #generated_images.append(addBounding(imread(pa+'/'+img_name)))
...
def load_generated_images_hole(images_folder):
    input_images = []
    target_images = []
    generated_images = []
    ...
    names = []
    for img_name in os.listdir(images_folder):
#        img = imread(os.path.join('/home/zjs/my/Pose-Transfer/cvprresults/fashion_PATN_1.0/test_latest/images', img_name))
        img = imread(os.path.join(images_folder, img_name))
        w = int(img.shape[1] / 8) #h, w ,c

        input_images.append(addBounding(img[:, :w]))
        target_images.append(addBounding(img[:, 3*w:4*w]))
        generated_images.append(addBounding(img[:, 7*w:]))
    ...
    return input_images, target_images, generated_images, names
def load_generated_images_hole1(images_folder):
    input_images = []
    target_images = []
    generated_images = []
    ...
    names = []
    for img_name in os.listdir(images_folder):
        if img_name.startswith('fashionW'):
            img = imread(os.path.join(images_folder, img_name))
            w = int(img.shape[1] / 9) #h, w ,c

            input_images.append(addBounding(img[:, :w]))
            target_images.append(addBounding(img[:, 3*w:4*w]))
            generated_images.append(addBounding(img[:, 8*w:]))
...
##    input_images, target_images, generated_images, names = load_generated_images_hole1(generated_images_dir)
##    input_images, target_images, generated_images, names = load_generated_images(generated_images_dir)
    input_images, target_images, generated_images, names = load_generated_images_hole(generated_images_dir)
...
if __name__ == "__main__":
    # fix these paths
#    generated_images_dir = './results/fashion_PATN_4/test_test1/images'
#    generated_images_dir = '/home/zjs/grade2/PInet/results/fashion_PInet_20_bak/test_test/images'
    generated_images_dir = '/home/zjs/grade2/PInet/results/fashion_PInet_withoutGan/test_latest/images'    

#    generated_images_dir =  '/home/zjs/grade2/PInet/result14/fashion_PInet_14_1/test_latest/images'
